 ...
 *      checking).
 * - Version 2.3, 05 Jun 2017.  Modified for use with mbed development model
 * 
 * @par Licence: GPLv3
...
{
	/* 1   2   4   8  16  32  64 */ // dataflash size index
	{  9,  9,  9,  9, 10, 10, 11 },  // m_buffersize /**< Size of the buffer address bits. **/
	{  9, 10, 11, 12, 12, 13, 13 },  // m_pageSize /**< Size of the page address bits. **/
	{  2,  3,  3,  4,  4,  6,  5 }   // m_sectorSize /**< Size of the sector address bits (part of the page address). **/
};

/**
 * Constructor
 * **/
DataFlash::DataFlash(PinName csPin, PinName resetPin, PinName wpPin)
	: m_spi(SPI_MOSI, SPI_MISO, SPI_SCK, csPin)
{
	m_chipSelectPin   = DigitalOut(csPin);
	if (resetPin != NC)
	{
		m_resetPin        = DigitalOut(resetPin);
		m_resetPin		  = HIGH; // set inactive
	}
	if (wpPin != NC)
	{
		m_writeProtectPin = DigitalOut(wpPin);
		m_writeProtectPin = HIGH; // set inactive
	}
	
}
DataFlash::DataFlash(PinName mosiPin, PinName misoPin, PinName sckPin, PinName csPin, PinName resetPin, PinName wpPin)
	: m_spi(mosiPin, misoPin, sckPin, csPin)
...
void DataFlash::setup()
{
	...
	//m_spi.format(8, 3);
	//m_spi.setBitOrder(MSBFIRST); //msb is default
	//m_spi.setClockDivider(SPI_CLOCK_DIV2); // default speed is 1mhz
...
uint8_t DataFlash::status()
{
    ...
    m_spi.write(DATAFLASH_STATUS_REGISTER_READ);
    /* Get result with a dummy write */
	status = m_spi.write(0);
...
void DataFlash::readID(struct DataFlash::ID &id)
{
    ...
    /* Send status read command */
	m_spi.write(DATAFLASH_READ_MANUFACTURER_AND_DEVICE_ID);

    /* Manufacturer ID */
	id.manufacturer = m_spi.write(0);
    /* Device ID (part 1) */
	id.device[0] = m_spi.write(0);
    /* Device ID (part 2) */
	id.device[1] = m_spi.write(0);
    /* Extended Device Information String Length */
    id.extendedInfoLength = m_spi.write(0);
...
void DataFlash::pageRead(uint16_t page, uint16_t offset)
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_PAGE_READ);
    
    /* Address (page | offset)  */
    m_spi.write(pageToHiU8(page));
    m_spi.write(pageToLoU8(page) | (uint8_t)(offset >> 8));
    m_spi.write((uint8_t)(offset & 0xff));
...
void DataFlash::arrayRead(uint16_t page, uint16_t offset)
{
    ...
    /* Send opcode */
#ifdef AT45_USE_SPI_SPEED_CONTROL
    m_spi.write(m_speed == SPEED_LOW ? DATAFLASH_CONTINUOUS_READ_LOW_FREQ :
                        DATAFLASH_CONTINUOUS_READ_HIGH_FREQ);
#else
    m_spi.write(DATAFLASH_CONTINUOUS_READ_LOW_FREQ);
#endif
...
void DataFlash::bufferRead(uint8_t bufferNum, uint16_t offset)
{
    ...
    if (bufferNum)
    {
        m_spi.write((m_speed == SPEED_LOW) ? DATAFLASH_BUFFER_2_READ_LOW_FREQ :
                                              DATAFLASH_BUFFER_2_READ);
    }
    else
    {
        m_spi.write((m_speed == SPEED_LOW) ? DATAFLASH_BUFFER_1_READ_LOW_FREQ :
                                              DATAFLASH_BUFFER_1_READ);

    }
#else
    m_spi.write(bufferNum ? DATAFLASH_BUFFER_2_READ_LOW_FREQ :
...
void DataFlash::bufferWrite(uint8_t bufferNum, uint16_t offset)
{
    ...
    m_spi.write(bufferNum ? DATAFLASH_BUFFER_2_WRITE :
...
void DataFlash::bufferToPage(uint8_t bufferNum, uint16_t page)
{
    ...
    {
        m_spi.write(bufferNum ? DATAFLASH_BUFFER_2_TO_PAGE_WITH_ERASE :
                                 DATAFLASH_BUFFER_1_TO_PAGE_WITH_ERASE);
    }
    else
    {
        m_spi.write(bufferNum ? DATAFLASH_BUFFER_2_TO_PAGE_WITHOUT_ERASE :
                                 DATAFLASH_BUFFER_1_TO_PAGE_WITHOUT_ERASE);
    }
    
    /* see pageToBuffer */
    m_spi.write(pageToHiU8(page));
    m_spi.write(pageToLoU8(page));
    m_spi.write(0x00);
...
void DataFlash::pageToBuffer(uint16_t page, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    m_spi.write(bufferNum ? DATAFLASH_TRANSFER_PAGE_TO_BUFFER_2 :
...
void DataFlash::pageErase(uint16_t page)
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_PAGE_ERASE);
...
void DataFlash::blockErase(uint16_t block)
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_BLOCK_ERASE);
...
void DataFlash::sectorErase(int8_t sector)
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_SECTOR_ERASE);
	
    if((sector == AT45_SECTOR_0A) || (sector == AT45_SECTOR_0B))
    {
        m_spi.write(0x00);
        m_spi.write((static_cast<uint8_t>(-sector) & 0x01) << (m_bufferSize - 5));
    }
    else
    {
        uint8_t shift = m_bufferSize + m_pageSize - m_sectorSize - 16;        
        m_spi.write(sector << shift);
        m_spi.write(0x00);
    }
...
void DataFlash::chipErase()
{
    ...
    m_spi.write(DATAFLASH_CHIP_ERASE_0);
    m_spi.write(DATAFLASH_CHIP_ERASE_1);
    m_spi.write(DATAFLASH_CHIP_ERASE_2);
    m_spi.write(DATAFLASH_CHIP_ERASE_3);
        ...
        uint16_t page, uint16_t offset, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    m_spi.write(bufferNum ? DATAFLASH_PAGE_THROUGH_BUFFER_2 :
...
int8_t DataFlash::isPageEqualBuffer(uint16_t page, uint8_t bufferNum)
{
    ...
    /* Send opcode */
    m_spi.write(bufferNum ? DATAFLASH_COMPARE_PAGE_TO_BUFFER_2 :
...
void DataFlash::deepPowerDown()
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_DEEP_POWER_DOWN);
...
void DataFlash::resumeFromDeepPowerDown()
{
    ...
    /* Send opcode */
    m_spi.write(DATAFLASH_RESUME_FROM_DEEP_POWER_DOWN);
