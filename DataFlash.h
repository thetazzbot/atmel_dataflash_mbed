 ...
 * - Copyright (C) 2017 by Mark Williamson @n
 * All rights reserved.
 *
 * @authors
 * - Volker Kuhlmann @n
...
#ifndef OUTPUT
#define OUTPUT PinDirection.PIN_OUTPUT
#endif

#ifndef INPUT
#define INPUT PinDirection.PIN_INPUT
#endif
 ...
 **/
/** Chip select (CS) **/
#define AT45_SS_PIN     SPI_CS
/** Reset (Reset) **/
#define AT45_RESET_PIN  NC
/** Write protect (WP) **/
#define AT45_WP_PIN     NC
/**
 * @}
 **/
...
class DataFlash
{
    ...
    public:
        /** Constructor **/
		DataFlash(PinName csPin, PinName resetPin, PinName wpPin);
		DataFlash(PinName mosiPin, PinName misoPin, PinName sclkPin,PinName csPin, PinName resetPin, PinName wpPin);
		...
		inline DigitalOut chipSelectPin() const;
        /** Get reset (RESET) pin **/
		inline DigitalOut resetPin() const;
        /** Get write protect (WP) pin **/
		inline DigitalOut writeProtectPin() const;
    ...
    private:
		...
		DigitalOut m_chipSelectPin=DigitalOut(SPI_CS);    /**< Chip select pin (CS). **/
		DigitalOut m_resetPin=DigitalOut(NC);         /**< Reset pin (RESET). **/
		DigitalOut m_writeProtectPin = DigitalOut(NC);  /**< Write protect pin (WP). **/
